[Vés al contingut](ca/la-fib/serveis-tic/copies-de-seguretat.md)
...
[ __](ca/la-fib/serveis-tic/copies-de-seguretat.md)
...
La FIB dóna un servei de còpies de seguretat de les dades que els usuaris
tenen en els nostres servidors.

## Utilització

Periòdicament es fan còpies de seguretat dels servidors de la FIB. Les dades
de les que es fa còpia de seguretat són els fitxers que tingueu en el vostre
espai d'usuari (directori _/dades_ a Linux i unitat _F:/_ a Windows) i dels
correus electrònics.  
  
Les còpies de seguretat es fan al final del dia, per tant, no es podrà
recuperar un fitxer que hagueu creat i esborrat dins el mateix dia.  
  
En cas de voler recuperar algun fitxer perdut accidentalment (i no el trobeu a
la paperera), cal contactar amb la FIB obrint un tiquet a
[https://peticions.utgcntic.upc.edu/tiquetspeticions/control/main?idEmpresa=103958](tiquetspeticions/control/main.md)
indicant:

  * El nom d'usuari.
  * El nom del servidor: per exemple si ha sigut a algun servidor unix, indiqueu el seu nom, o si ha sigut al servidor de correu o als entorns de les aules windows o linux. 
  * Directori i nom del fitxer. 
  * Dates de creació i del dia del que voleu recuperar el fitxer.

La recuperació no es realitza al moment ja que és un procés manual, i per tant
pot tardar 24-48 hores en dies laborables.

## FAQ's

__**He reiniciat un PC dels laboratoris i el fitxer que tenia no apareix
enlloc.**

Els fitxers que segur es conserven quan reinicieu PC dels laboratoris són els
que es guarden en el vostre espai d'usuari. Per exemple, guardar arxius a
l'escriptori de windows pot provocar que el perfil es corrompi i per tant els
fitxers quedin inaccessibles. El mateix passa amb el /home de linux.  
Recordeu que només es fa còpia de seguretat del directori /dades (Linux) i de
la unitat F: (Windows).
...
[Configuració de privadesa](ca/la-fib/serveis-tic/copies-de-seguretat.md)
