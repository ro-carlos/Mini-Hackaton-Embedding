[Vés al contingut](ca/la-fib/serveis-tic/examens-en-entorn-segur.md)
  ...
  * [Català](ca/la-fib/serveis-tic/examens-en-entorn-segur.md "Exàmens en entorn segur")
  * [English](en/fib/it-services/exams-secure-environment.md "Exams in a Secure Environment")
  * [Español](es/la-fib/servicios-tic/examenes-en-entorno-seguro.md "Exámenes en entorno seguro")
...
[ __](ca/la-fib/serveis-tic/examens-en-entorn-segur.md)
...
L'entorn segur ens permet restringir l'accés a Internet en els laboratoris
d'informàtica, de manera que es poden realitzar classes o exàmens en unes
condicions més controlades, tant per Windows com per Linux.  

## Utilització

La FIB posa a disposició dels professors un servei de lliurament d'exàmens de
laboratoris en entorn segur, pensat per totes aquelles assignatures que volen
fer una classe o un examen en un entorn restringit.

Aquest servei es basa en uns filtres que impedeixen que els ordinadors del
laboratori on es fa l'examen es puguin connectar a Internet, exceptuant
determinades pàgines web (com la de lliurament d'examens del Racó) i serveis.

Aquests són els punts a tenir en compte per utilitzar l'entorn segur:

  1. La FIB ha de saber amb la suficient antelació (com a mínim una setmana) quan i en quin laboratori es realitzarà l'examen.
  2. La imatge de Linux serà la mateixa que la que es fa servir habitualment a les aules, les úniques diferencies seran: 
     * Estarà protegida per a que l'alumne no pugui realitzar cap connexió a l'exterior o qualsevol màquina o servei de dins de la UPC, excepte els serveis necessaris per a fer la classe (servidors de BD, gitlab, ...).
     * Només hi haurà disponible una única connexió al servidor de fitxers (el /dades o la unitat F:) i una altra a la URL de lliurament d'exàmens del Racó (o altre pàgina web d'entrega d'examens del departament).
     * No es podran fer servir dispositius USB ni la unitat de CDROM en les màquines Linux. Aquest punt és opcional, si es vol l'accés s'ha de demanar.
  3. L'entorn segur s'activarà a l'hora indicada per l'examen i es desactivarà 5 minuts després de l'hora indicada del final de l'examen.
  4. En cas de fer servir la pàgina web d'examens del Racó per a fer l'entrega cal saber que: 
    1. El professor haurà de donar d'alta l'examen a l'aplicació de lliurament de pràctiques del Racó del Professor. S'haurà d'especificar l'aula i les hores d'inici i de final de la pràctica en que es farà el lliurament perquè ningú extern a aquesta aula pugui lliurar-lo. En el moment de crear una pràctica, aquesta ja serà visible pels alumnes, però es podrà amagar posant-la a invisible des d'aquesta mateixa pantalla.
    2. Els estudiants s'hauran de connectar a [https://examens.fib.upc.edu](index.md) amb el seu nom d'usuari i contrasenya del Racó i prémer el botó de "lliurar pràctica". En la majoria dels casos, això ja els portarà directament a la pràctica/examen que han de lliurar. Si no fos així, hauran de triar la pràctica a lliurar d'una llista.
    3. De cara a detectar possibles incidències, quan el professor reculli les pràctiques tindrà en el fitxer .zip un fitxer index.txt en que quedarà especificat per cada lliurament: el nom, fitxers, hora de lliurament i des de quin ordinador s'ha fet. D'aquesta manera es podrà detectar que cada ordinador només tingui un únic lliurament.

El dia de l'examen, a l'hora d'inici indicada s'activa l'entorn segur
automàticament i els estudiants ja poden encendre els ordinadors i entrar al
Windows o Linux.

**L'entorn segur no s'ha de confondre amb la imatge especial de Linux per
examens**. Aquesta imatge s'anomena "Linux Exàmens" i es pot seleccionar a la
pantalla inicial de selecció d'imatges dels ordinadors. És una imatge de Linux
igual que el normal (mateix programari instal·lat) però on els estudiants no
poden accedir al seu compte, sino que es connecten amb comptes locals i on
l'accés a Internet ja està deshabilitat (exceptuant les pàgines web de
lliurament de pràctiques). L'entorn segur es pot combinar amb la imatge "Linux
examens".

Al Racó del professor, a l'apartat "Documentació serveis TIC per a professors"
teniu més informació sobre els usuaris per a accedir a aquesta imatge.

Si teniu qualsevol dubte i/o per a realitzar una petició d'activació d'Entorn
Segur per a un examen, podeu obrir un tiiquet a
[https://peticions.utgcntic.upc.edu/tiquetspeticions/control/main?idEmpresa=103958](tiquetspeticions/control/main.md)
indicant el dia, les hores i la/es aula/es en la que ha d'estar activat
juntament amb tota aquella informació que cregueu oportuna.

## FAQ's

__**Si he demanat entorn segur per un examen, el PC del professor tindrà accés
lliure a Internet?**

No, l'entorn segur s'activa per tota l'aula i el PC del professor pertany a
l'aula, per tant no hi haurà accés a Internet. Si necessiteu tenir accés a
Internet durant l'exàmen, podeu portar un ordinador portàtil o bé demanar-lo
en préstec al Servei d'Operació.
...
[Configuració de privadesa](ca/la-fib/serveis-tic/examens-en-entorn-segur.md)
